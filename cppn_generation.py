...
def generate_block(net, corner, args, block_options, scaled_point, change): 
    ...
    if output[0] >= presence_threshold: 
        output_val = util.argmax(output[1:len(block_options)+1])
    ...
    if args.EVOLVE_SNAKE:
        directions_preferences = output[len(block_options):len(block_options)+NUM_DIRECTIONS]
        direction_index = util.argmax(directions_preferences)
        ...
        stop = output[len(block_options)+NUM_DIRECTIONS] <= args.CONTINUATION_THRESHOLD
